...
-- | The global event manager. Typically used like
--
-- @
-- func :: IO ()
-- func = do
--   ...
--   let !mngr = Emmy.manager
--   ...
-- @
manager :: Manager
{-# noinline manager #-}
...
-- The file descriptor must already be registered.
-- Otherwise, this function may look in an
-- uninitialized tier-two array.
lookupGeneric ::
     Int -- Read: 0, Write: 1
  -> Int -- File descriptor
...
casMutableUnliftedArrayArray (MutableUnliftedArray arr#) (I# i#) (MutableUnliftedArray old) (MutableUnliftedArray new) =
  ...
  -- Also, this is unsound if the result is immediately consumed by
  -- the FFI.
  IO $ \s0 ->
